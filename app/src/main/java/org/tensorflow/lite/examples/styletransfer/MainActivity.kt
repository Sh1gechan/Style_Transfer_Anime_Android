...
class MainActivity :
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val styles = arrayOf("Paprika Style", "Hayao Style", "selfie2anime", "anime_sketch", "open_sketch", "contour_style")
  ...
  private val parentDirectory: File by lazy {
    File(getExternalFilesDir(null), "StyleTransfer").apply {
      mkdirs()
    }
  }
  ...
  // 変換後の画像を保存
  fun saveStyledImage() {
    val resultImageDrawable = resultImageView.drawable
    if (resultImageDrawable is BitmapDrawable) {
      val styledImage = resultImageDrawable.bitmap

      val outputStream: OutputStream?
      val resolver = contentResolver
      val contentValues = ContentValues().apply {
        put(MediaStore.MediaColumns.DISPLAY_NAME, "styled_image.jpg")
      ...
      try {
        ...
        outputStream?.use { outputStream ->
          // スタイル変換後の画像を保存
          styledImage.compress(Bitmap.CompressFormat.JPEG, 100, outputStream)
